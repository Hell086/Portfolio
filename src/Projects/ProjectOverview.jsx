...
const getCaseStudy = (project) => {
  const { finalOutcome, ...sections } = project.caseStudyContent || {};

  return {
    sections: Object.values(sections).filter(section => section?.title && section?.content),
    finalLabel: finalOutcome?.title || 'Final Outcome',
    finalVideo: finalOutcome?.video,
  };
};
...
const ProjectOverview = () => {
  ...
  const supportsFinalVideo = project && ['Animation', 'Motion Graphics'].includes(project.category);
  const hasFinalMedia = !supportsFinalVideo || Boolean(caseStudy.finalVideo);
  ...
  return (
    <div className="min-h-screen bg-neutral-50 pt-12 sm:pt-14">
          ...
          >
            ...
            {hasFinalMedia && (
              <section className="border-t border-neutral-200 pt-10">
                ...
                {supportsFinalVideo ? (
                  <video
                    className="w-full aspect-video rounded-2xl border border-neutral-300 bg-white shadow-sm"
                    controls
                    playsInline
                    preload="metadata"
                  >
                    <source src={caseStudy.finalVideo} />
